...
;; JTK - make it SHUT UP at load time
(eval-when (:load-toplevel :compile-toplevel :execute) 
  (defparameter *verbose-messages* nil))

(progn
  (when *verbose-messages* (format t "~&;;; Checking for wide character support..."))
  (force-output)
  (flet ((test (code)
	   (and (< code char-code-limit) (code-char code))))
    (cond
      ((not (test 50000))
       (when *verbose-messages* (format t " no, reverting to octet strings.~%"))
       #+rune-is-character
	 ...
	 (when *verbose-messages*
       ...
       (when *verbose-messages* (format t " yes, using code points.~%"))
       #+(or rune-is-integer rune-is-utf-16)
       (error "conflicting unicode configuration.  Please recompile.")
       (pushnew :rune-is-character *features*))
      (t
       (when *verbose-messages* (format t " yes, using UTF-16.~%"))
...
(asdf:defsystem :cxml-jtk/dom
    ...
    :depends-on (:cxml-jtk/xml))

(asdf:defsystem :cxml-jtk/klacks
    :default-component-class closure-source-file
...
(asdf:defsystem :cxml-jtk/test
    ...
    :depends-on (:cxml-jtk/xml :cxml-jtk/klacks :cxml-jtk/dom))

(asdf:defsystem :cxml-jtk
    :components ()
    :depends-on (:cxml-jtk/dom :cxml-jtk/klacks #-allegro :cxml-jtk/test))
