...
;; linear interpolation - single precision (linterpF package)
...
(defstruct linterp-struct
  ...
  (y-vec nil :type (or null (simple-array single-float)))
  ...
  (x-min-val nil :type (or null single-float))
  (x-max-val nil :type (or null single-float)))

;; reverses vectors x,y if necessary so elements in x are in
;; increasing order
(defun force-increasing-order (x-vec y-vec)
  (declare (type (simple-array single-float (*)) x-vec y-vec)
...
(defun fix-input-arrays (x-vec y-vec)
  ...
  (let ((need-reverse (< (aref x-vec 1) (aref x-vec 0))))
    ...
    ;;force copy if we need to reverse
    (setf y-vec (make-flt-array y-vec need-reverse)) 
...
(defun make-linterp (x-vec y-vec &key x-min-val x-max-val)
  ...
  (make-linterp-struct
   ...
   :x-min-val (if x-min-val (coerce x-min-val 'single-float))
   :x-max-val (if x-max-val (coerce x-max-val 'single-float))))
  ...
  (locally
   ...
   (let ((klo 0)
     ...
     (declare (fixnum klo khi k n)
	      ...
	      (type (simple-array single-float) xa ya))
     (when (< x xmin)
     ...
     (let ((x0 (aref xa klo))
	   (y0 (aref ya klo))
	   (y1 (aref ya khi)))
       (declare (type single-float x0 y0 y1))
